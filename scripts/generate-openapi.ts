...
/**
 * Africa Payments MCP - OpenAPI Spec Generator
 * 
 * Usage:
 *   npx ts-node scripts/generate-openapi.ts
 * 
 * Generates openapi.yaml from the TypeScript types and MCP tools
 */
...
function generateOpenAPISpec(): object {
  return {
    ...
    info: {
      ...
      description: `Unified API for African payment providers - M-Pesa, Paystack, MTN MoMo, Airtel Money, and more.
...
`,
    ...
    tags: [
      { name: 'Payments', description: 'Core payment operations' },
      { name: 'Transactions', description: 'Transaction management and verification' },
      { name: 'Refunds', description: 'Refund operations' },
      { name: 'Providers', description: 'Provider information and management' },
      { name: 'Webhooks', description: 'Webhook handling and verification' },
    ...
    '/payments/request': {
      post: {
        ...
        requestBody: {
          ...
          content: {
            'application/json': {
              schema: { $ref: '#/components/schemas/RequestPaymentRequest' }
            }
          }
        },
        responses: {
...
async function main() {
  ...
  const outputPath = path.join(ROOT_DIR, 'openapi.yaml');
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (value === null) {
        ...
        if (value.length === 0) {
          ...
          yaml += `${spaces}${formattedKey}: [${value.map(formatValue).join(', ')}]\n`;
        } else {
